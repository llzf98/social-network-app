...
func CheckUser(username, password string) (bool, error) { // username is also string
	// ReadES based on username
	// compare the given password with password returned from ES

	// ReadES based on both username and password
	// check if there's a hit
	
	// combine two item query into a single bool query
...
func AddUser(user *model.User) (bool, error) {
	// elasticsearch insert: insert + update
